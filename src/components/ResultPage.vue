...
const weightSum = computed(() =>
  props.logData.map((l) => Number(l["weight"])).reduce((a, b) => a + b, 0)
);
      ...
      <div class="grid gap-4 grid-cols-2">
        <section class="flex flex-col items-center">
          ...
          <div class="p-4">
            <PercentCircle
              show-value
              class="w-full h-full"
              foreground-circle-class="text-green-600"
              :percentage="repSum / targetRepsSum"
            ></PercentCircle>
          </div>
        ...
        <section class="flex flex-col items-center">
          ...
          <div class="flex flex-1 h-full items-center justify-center">
            <ChevronRightIcon
              v-if="weightDirection == 'same'"
              class="w-2/3 text-orange-600"
            />
            <ChevronUpIcon
              v-if="weightDirection == 'up'"
              class="w-2/3 text-green-600"
            />
            <ChevronDownIcon
              v-if="weightDirection == 'down'"
              class="w-2/3 text-red-600"
            />
          </div>
          <span
            >{{ weightSum }} (
